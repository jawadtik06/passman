...
use crate::errors::{Result, PassManError};
...
impl Database {
    pub fn new(path: &str) -> Result<Self> {
        ...
        conn.execute("CREATE INDEX IF NOT EXISTS idx_website ON passwords(website)", [])?;
    ...
    pub fn get_by_id(&self, id: i64) -> Result<Option<PasswordEntry>> {
        ...
        rows.next().transpose().map_err(|e| PassManError::DatabaseError(e.to_string()))
    }
    ...
    pub fn count(&self) -> Result<usize> {
        let count: i64 = self.conn.query_row("SELECT COUNT(*) FROM passwords", [], |row| row.get(0))?;
        Ok(count as usize)
    }
